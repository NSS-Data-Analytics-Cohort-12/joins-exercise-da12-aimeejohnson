...
SELECT ROUND(release_year, -1) AS decade, SUM(worldwide_gross) AS total_gross
FROM specs
INNER JOIN revenue
...
SELECT film_title, TRIM(TRAILING ' 2' FROM film_title) AS original_title
FROM specs
...
SELECT SPLIT_PART(film_title,':',1) AS movie_series, COUNT(*) as series_count
FROM specs
...
SELECT SPLIT_PART(film_title,':',1) AS movie_series, COUNT(*) as series_count, AVG(imdb_rating) AS average_rating
FROM specs
INNER JOIN rating
...
--Highest = The Lord of the Rings, Lowest = The Twighlight Saga
...
SELECT company_name, MAX(film_title) AS film_title, MAX(imdb_rating) AS highest_rating
FROM distributors
LEFT JOIN LATERAL(
	SELECT film_title, imdb_rating
	FROM specs
	INNER JOIN rating ON specs.movie_id = rating.movie_id
	WHERE specs.domestic_distributor_id = distributors.distributor_id
) AS newtable ON true
